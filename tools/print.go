...
func SshPrintFiles(email, password string, files []string, deleteAfter bool, oneSided bool, noOfCopies int) {
	parts := strings.Split(email, "@")
	if len(parts) != 2 {
		fmt.Println("Invalid email format")
		os.Exit(1)
	...
	for _, file := range files {
		filename := "'" + filepath.Base(file) + "'"
		lprCmd := fmt.Sprintf("lpr -P %s %s %s %s%s", selectedPrinter,
			func() string {
				if oneSided {
			...
			func() string {
				if noOfCopies > 1 {
					return fmt.Sprintf("-# %d", noOfCopies)
				}
				return ""
			}(),
			func() string {
